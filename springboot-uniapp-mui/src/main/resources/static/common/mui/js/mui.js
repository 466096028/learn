...
(function ($, window) {
            ...
            for (; target && target !== document; target = target.parentNode) {
                if (target.tagName === 'A' && target.hash) {
                    ...
                    if (page && page.classList.contains(CLASS_PAGE)) {
                        ...
                        //设置标题、返回按钮
                        HuanziHeader.title($(".mui-page-center")[0].dataset.title);
                        HuanziHeader.showBackButton(function () {
                            HuanziHeader.hideBackButton();
                            HuanziHeader.title($(".mui-page-left")[0].dataset.title);
                            View.back();
                        });
                        break;
                    }
    ...
    $.fn.view = function (options) {
        ...
        if (!id) {
            id = ++$.uuid;
            $.data[id] = viewApi = View.init(self, options);
